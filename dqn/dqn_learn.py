...
def dqn_learing(
    ...
    for t in count():
        ...
        action = select_epilson_greedy_action(Q, obs, t, Statistic['mean_episode_reward'][max(0,t-1)], Statistic['mean_episode_reward'][max(0,t-2)])
        #Step environment
        obs, reward, done, info = env.step(action)
        #Store effect
        ...
        if (t > learning_starts and
            ...
            if replay_buffer.can_sample(batch_size):
                ...
                obs_batch, act_batch, rew_batch, next_obs_batch, done_mask = batch_transitions
                #Turn numpy arrays to torch variables
                obs_batch = Variable(torch.from_numpy(obs_batch).type(dtype) / 255.0)
                act_batch = Variable(torch.from_numpy(act_batch).long())
                rew_batch = Variable(torch.from_numpy(rew_batch))
                next_obs_batch = Variable(torch.from_numpy(next_obs_batch).type(dtype) / 255.0)
                done_mask = Variable(torch.from_numpy(done_mask))
                #Use cuda if available
                if USE_CUDA:
                ...
                optimizer.step()
                num_param_updates += 1
                #Update target network if reached target_update_freq
